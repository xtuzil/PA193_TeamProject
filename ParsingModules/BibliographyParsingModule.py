...
class BibliographyParsingModule(IParsingModule):
    ...
    _bibliography_title_regex_string = r"[\d\s.]*(?:References|Referenced Literature|Bibliography|REFERENCE DOCUMENTS|Literature)" 
    _bibliography_regex_string = r'\[.{1,20}\]\n* {1,50}[“\w].*\n(?: {1,50}[\(“\w].*(?:\n)+)*'


    @staticmethod
    def _change_newline(string):
        change_newlines_to_space = re.sub(r"\n\s+", " ", string)
        return re.sub("\n", "", change_newlines_to_space)
    
    @staticmethod
    def _is_bib_on_page(certificate, page_number) -> bool:
        ...
        if founded != None:
            if re.search(BibliographyParsingModule._bibliography_regex_string, certificate.get_page_content(page_number)) == None:
                return False
            else:
                return True
        return False
    ...
    @staticmethod
    def _guess_bibliography_length(certificate: Certificate, start):
        min_b = 2
        max_b = 5
        actual_guess = 0
        for page_number in range(start, certificate.get_pages_count()+1):
            founded_bibliography = re.search(BibliographyParsingModule._bibliography_regex_string, certificate.get_page_content(page_number))
            if founded_bibliography == None:
    ...
    def _find_bibliography_start_page_number(certificate: Certificate) -> int:
        ...
        for page_number in range(certificate.get_pages_count(), 0, -1):
            if (BibliographyParsingModule._is_bib_on_page(certificate, page_number)) and not (BibliographyParsingModule._is_bib_on_page(certificate, page_number-1)):
    ...
    def parse(certificate: Certificate):
        ...
        #parse each bibliograpy - append to dictionary key:value
